...
Elm.Chat.make = function (_elm) {
   ...
   var viewMsgList = function (msgList) {
      return function () {
   ...
   "Chat.Msg",
   function (v) {
      return typeof v === "object" && "username" in v && "content" in v ? {_: {}
                                                                          ,username: typeof v.username === "string" || typeof v.username === "object" && v.username instanceof String ? v.username : _U.badPort("a string",
                                                                          v.username)
                                                                          ,content: typeof v.content === "string" || typeof v.content === "object" && v.content instanceof String ? v.content : _U.badPort("a string",
   ...
   var InputMsg = function (a) {
      return {ctor: "InputMsg"
             ,_0: a};
   };
   var NewMsg = function (a) {
      return {ctor: "NewMsg"
             ,_0: a};
   };
   var incMsgActions = A2($Signal.map,
   function (m) {
      return NewMsg(m);
   },
   incMsg);
   var NoOp = {ctor: "NoOp"};
   var sendMessage = F2(function (msg,
   key) {
      return _U.eq(key,
      13) ? SendMsg(msg) : NoOp;
   });
   ...
   model) {
      ...
      _L.fromArray([A2($Html.div,
                   ...
                   _L.fromArray([A2($Html.input,
                                ...
                                ,A2($Html.input,
                                _L.fromArray([$Html$Attributes.$class("col-xs-9")
                                             ,$Html$Attributes.value(model.inputMsg)
                                             ,A3($Html$Events.on,
                                             "input",
                                             $Html$Events.targetValue,
                                             function ($) {
                                                return $Signal.message(address)(InputMsg($));
                                             })
                                             ,A2($Html$Events.onKeyUp,
                                             address,
                                             sendMessage(mkMessage(model)))]),
                                _L.fromArray([]))]))]));
   });
   var actions = $Signal.mailbox(NoOp);
   var outMsg = Elm.Native.Port.make(_elm).outboundSignal("outMsg",
   function (v) {
      return {username: v.username
             ,content: v.content};
   },
   function () {
      var mapToMsg = function (action) {
         return function () {
            switch (action.ctor)
            {case "SendMsg":
               return action._0;}
      ...
      var msgOnly = function (a) {
         return function () {
            switch (a.ctor)
            {case "SendMsg": return true;}
            return false;
         }();
      };
      var onlySendMsg = A3($Signal.filter,
      msgOnly,
      NoOp,
      actions.signal);
...
var _elm_chat = require("./elm_chat");
...
var games = _socket2["default"].channel("games:ID_GRY", { player_id: 123 });
...
games.on("back:modelUpdate", function (model) {
  console.log("back:modelUpdate", model);
});
});

;require.register("web/static/js/elm_chat", function(exports, require, module) {
"use strict";
...
function initElmChat(domElement, chatUUID) {
  ...
  // Push into elm ports
  chats.on("back:msg", function (msg) {
